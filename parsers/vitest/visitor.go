...
import (
	...
	specantlr "github.com/jmewes/slicer/parsers/vitest/antlr"
)
...
// specVisitor walks the ANTLR parse tree of a Vitest-style spec file and
// builds a tree of *frame values. Modifier chains (`.skip` / `.only` /
// `.todo` / `.each` / `.for`) are recognized by the grammar but ignored
// here: they do not change the mapping to Gherkin. Top-level `test`/`it`
// calls (with no enclosing `describe`) and top-level `beforeEach`/
// `beforeAll` calls are collected separately so the emitter can synthesize
// an implicit root Feature for them.
type specVisitor struct {
	tokens            *antlr.CommonTokenStream
	relaxedMode       bool
	topLevelDescribes []*frame
	describeStack     []*frame
	rootScenarios     []*messages.Scenario
	rootBackground    []*messages.Step
}
...
// visitSuite handles a describe/it/test/beforeEach/beforeAll/afterEach/
// afterAll call: it opens the appropriate frame, walks its callback block
// for nested suites, and then closes the frame. Modifier chains
// (`suiteKeyword modifier*`) are intentionally not inspected.
func (v *specVisitor) visitSuite(s specantlr.ISuiteContext) {
	...
	case frameIt:
		...
		if enclosing := v.enclosingDescribe(); enclosing != nil {
			...
			v.rootScenarios = append(v.rootScenarios, scenario)
		}
	...
	case frameBefore:
		...
		if len(bgSteps) > 0 {
			if enclosing := v.enclosingDescribe(); enclosing != nil {
				...
				v.rootBackground = bgSteps
			}
		}

	case frameAfter:
		// afterEach/afterAll bodies are recognized (so their contents are
		// not walked for nested suites in a misleading way) but their
		// steps are discarded — they produce no Gherkin output.
	}
}
...
func suiteKind(k specantlr.ISuiteKeywordContext) string {
	...
	if k.IT() != nil || k.TEST() != nil {
		return frameIt
	}
	if k.BEFOREEACH() != nil || k.BEFOREALL() != nil {
		return frameBefore
	}
	if k.AFTEREACH() != nil || k.AFTERALL() != nil {
		return frameAfter
	}
	return ""
}
